    ...
    <td width="150">
        @if ($showTrashButtons)
            <form action="{{ route('contacts.restore', $contact->id) }}" method="POST" style="display:inline">
                @csrf
                @method('delete')
                <button type="submit" class="btn btn-sm btn-circle btn-outline-info" title="Delete"><i class="fa fa-undo"></i></button>
            </form>  
            <form action="{{ route('contacts.force-delete', $contact->id) }}" onsubmit="return confirm('Your data will be remove permanently')" method="POST" style="display:inline">
                @csrf
                @method('delete')
